...
class GroupDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sequence = [random.choice(list(range(1, 1 + 2 * self.generators))) for _ in range(length)]
...
def pairwise_distances(sequences):
    ...
    for i in range(batch_size):
        for j in range(i + 1):
            result[i, j] = result[j, i] = lev.distance("".join(map(str, sequences[i])), "".join(map(str, sequences[j])))
