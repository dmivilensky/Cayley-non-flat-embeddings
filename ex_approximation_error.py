...
if not os.path.isfile(FIGURES_PATH + f'approx_{epochs}_{generators}_{small}.data'):
    batch_size = 50
    ...
    scheduler = WarmupMultiStepLR(optimizer, warmup_iters=10, gamma=0.999)
    ...
    losses = []
    for big in tqdm(range(2, small + 6)):
    ...
    with open(FIGURES_PATH + f'approx_{epochs}_{generators}_{small}.data', 'wb') as f:
        pickle.dump(losses, f)

else:
    with open(FIGURES_PATH + f'approx_{epochs}_{generators}_{small}.data', 'rb') as f:
...
ax.plot(list(range(2, small + 6)), losses)
ax.set_xlabel('$R$')
ax.set_ylabel('$f_R(x^N)$')
ax.set_title(f'$d = {dimension}, gen = {generators}, r = {small}$')
ax.grid(alpha=0.4)
plt.tight_layout()
plt.savefig(FIGURES_PATH + f'approx_{epochs}_{generators}_{small}.pdf')
plt.show()
